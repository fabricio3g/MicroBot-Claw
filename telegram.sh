...
# Parse updates and process messages
tg_process_updates() {
    local resp="$1"
    
    # Check for valid response
    if [ -z "$resp" ]; then
        echo "[telegram] No response from API"
        return
    fi
    ...
    local count=$(echo "$resp" | jsonfilter -e '@.result[*]' 2>/dev/null | wc -l)
    ...
    local i=0
    while [ $i -lt $count ]; do
        local update_id=$(echo "$resp" | jsonfilter -e "@.result[$i].update_id" 2>/dev/null)
        
        # Update offset
        if [ -n "$update_id" ] && [ "$update_id" -ge "$UPDATE_OFFSET" ]; then
            UPDATE_OFFSET=$((update_id + 1))
        fi
        
        # Extract message
        local chat_id=$(echo "$resp" | jsonfilter -e "@.result[$i].message.chat.id" 2>/dev/null)
        local text=$(echo "$resp" | jsonfilter -e "@.result[$i].message.text" 2>/dev/null)
        local username=$(echo "$resp" | jsonfilter -e "@.result[$i].message.from.username" 2>/dev/null)
